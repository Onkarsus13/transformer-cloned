...
class DinoV3VisionEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.processor = AutoImageProcessor.from_pretrained(vision_model_name)
        base_model = AutoModel.from_pretrained(vision_model_name, config=config)
    ...
    def forward(self, pixel_values: torch.Tensor) -> Tuple[torch.Tensor, Tuple[int, int]]:
        ...
        outputs = self.model(pixel_values=flat_pixels, output_hidden_states=True, return_dict=True)
...
class DinoV3TextEncoder(nn.Module):
    ...
    @property
    def hidden_size(self) -> int:
        return getattr(self.model.config, "hidden_size", None) or getattr(self.model.config, "d_model")
...
class DinoV3ParallelVideoTextMatcher(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.vision_encoder = DinoV3VisionEncoder(
            ...
            vision_lora_path=vision_lora_path,
        )
        self.vision_model = self.vision_encoder.model

        self.text_encoder = DinoV3TextEncoder(
            text_model_name=text_model_name,
            freeze_parameters=freeze_text_encoder,
        )
    ...
    def encode_video(self, pixel_values: torch.Tensor) -> Tuple[torch.Tensor, Tuple[int, int]]:
        patch_states, spatial_shape = self.vision_encoder(pixel_values)
        patch_states = self.patch_norm(patch_states)
        return patch_states, spatial_shape
    ...
    def save_pretrained(self, save_directory: Union[str, Path]) -> None:
        ...
        model_state = {}
        skip_prefixes = ("vision_model.", "text_model.", "vision_encoder.", "text_encoder.")
    ...
    def from_pretrained(
        ...
        **overrides: Any,
        ...
        if state_path.exists():
            ...
            skip_prefixes = ("vision_model.", "text_model.", "vision_encoder.", "text_encoder.")
            missing = [key for key in missing if not key.startswith(skip_prefixes)]
            if missing:
